...
async def upload_page(form_id: str, form_uuid: str, form_title: str):
    """
    ...
    log_lines = []
    log_view = None
    LOG_FILE = f"upload_log_{form_id}.log"
    ...
    def download_log():
        if os.path.exists(LOG_FILE):
            with open(LOG_FILE, "rb") as f:
                log_bytes = f.read()
            ui.download(log_bytes, LOG_FILE)
        else:
            ui.notify("No log file found.", type="warning")

    with ui.element('div').classes('columns-2 gap-2 h-full w-full'):
        TAILWIND_CARD = 'w-full items-center justify-center h-full mb-2 p-2 break-inside-avoid h-1/2'
...
async def forms_page():
    ...
    def handle_form_selection(e: events.TableSelectionEventArguments):
        ...
        form_uuid = row['uuid']
        ui.navigate.to(
            f'/upload?form_id={form_id}&form_uuid={form_uuid}&form_title={form_title}')
